...
async function processNamespace(opts: TSG.ProcessNSOpts) {
  ...
  for (let _class of namespace.classes) {
    //TODO pack all classes inside a module if namespace has a name
...
function getImportsFromDeps(opts: TSG.GenClassNameOpts): ts.Statement[] {
  ...
  return _class.dependencies.filter((dep) => {
    if (_class.name === dep.name) return false;
    return dep.name.indexOf("Glide") === 0;
  }).map(dep => {
